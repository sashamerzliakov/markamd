...
| Editable code files | click a .js / .mjs / .cjs / .css / .py / .json / .log / .env file | Opens directly in the editor as plain text (editor-only — no markdown preview), with normal save (⌘S). `.env` / `.env.*` are also made **visible** in the explorer (upstream hides all dotfiles — a deliberate fork divergence, incl. the adjusted upstream test in tests/files.test.ts). |
