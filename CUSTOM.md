...
| In-app file viewer | click an image / PDF / video / audio file in the sidebar | Opens as a read-only viewer **tab** (current tab is preserved); Esc or × closes the tab. PDFs use WKWebView's native viewer; video (.mp4/.m4v/.mov/.webm/.ogv) and audio (.mp3/.m4a/.wav/.ogg/.flac) play with native controls (512MB cap). SVG viewer has an "edit as text" button. Office files (.doc/.docx/.xls/.xlsx/.ppt/.pptx) get a launcher card → default app. |
...
| Editable code files | click a .js / .mjs / .cjs / .ts / .tsx / .jsx / .css / .py / .json / .yaml / .yml / .toml / .ini / .conf / .sh / .zsh / .xml / .txt / .log / .rtf / .env file | Opens directly in the editor as plain text (editor-only — no markdown preview), with normal save (⌘S). `.rtf` opens as raw RTF markup (no renderer). `.env` / `.env.*` are also made **visible** in the explorer (upstream hides all dotfiles — a deliberate fork divergence, incl. the adjusted upstream test in tests/files.test.ts). |
